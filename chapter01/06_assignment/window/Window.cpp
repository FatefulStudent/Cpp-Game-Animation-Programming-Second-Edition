...
bool Window::init(unsigned int width, unsigned int height, std::string title) {
  ...
  glfwSetWindowSizeCallback(mWindow, [](GLFWwindow *win, int width, int length) {
    auto thisWindow = static_cast<Window*>(glfwGetWindowUserPointer(win));
    thisWindow->handleWindowResizedEvents(width, length);
    }
  );
...
void Window::handleWindowResizedEvents(int width, int length){
  Logger::log(1, "%s: Window has been resized to %i/%i\n", __FUNCTION__, width, length);
}
